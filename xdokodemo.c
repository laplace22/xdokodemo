...
int main(int argc, char *argv[], char *envp[]) {
    ...
    //コマンドラインオプションチェック
    for(args_ptr=1;args_ptr<argc;args_ptr++){
        if(argv[args_ptr][0]=='-'){
            switch(argv[args_ptr][1]){
            case 'f'://from アドレスチェック
                if(argv[args_ptr][2]=='\0'){
                    char_ptr=argv[args_ptr+1];
                    args_ptr++;
                }else{
                    char_ptr=argv[args_ptr]+2;
                }
                if (strchr(char_ptr,'/')){
                    //maskあり
                    slash_ptr=atoi(strchr(char_ptr,'/')+1);
                    if (slash_ptr<1||slash_ptr>32){
                        perror("Ip address error.(mask)");
                        return 1;
                    }
                    mask.s_addr=-1<<(32-slash_ptr);
                    *strchr(char_ptr,'/')='\0';
                    if (!inet_aton(char_ptr,&net)){
                        printf("%s",char_ptr);
            ...
            case 'p'://ポート指定
                if(argv[args_ptr][2]=='\0'){
                    port=atoi(argv[args_ptr+1]);
                    args_ptr++;
                }else{
                    port=atoi(argv[args_ptr]+2);
                }
    ...
    while(1) {
        //受信処理
        packet_size=sizeof(struct sockaddr_in);
        numrcv=recvfrom(recvSocket,buffer,(size_t)BUFFER_SIZE,0,(struct sockaddr*)&sockAddr,(socklen_t *)&packet_size);
        if(numrcv==-1){
